...
public class GongzuoliuController  extends JavaEEFrameworkBaseController<Gongzuoliu> implements Constant {
        ...
        if (StringUtils.isNotBlank(filters)) {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                if (result.getString("field").equals("dictKey") && result.getString("op").equals("eq")) {
                    gongzuoliu.set$eq_dictKey(result.getString("data"));
                }
                if (result.getString("field").equals("dictValue") && result.getString("op").equals("cn")) {
                    gongzuoliu.set$like_dictValue(result.getString("data"));
                }
            }
            if (((String) jsonObject.get("groupOp")).equalsIgnoreCase("OR")) {
                gongzuoliu.setFlag("OR");
            } else {
                gongzuoliu.setFlag("AND");
            }
        }
        gongzuoliu.setFirstResult((firstResult - 1) * maxResults);
        gongzuoliu.setMaxResults(maxResults);
        Map<String, String> sortedCondition = new HashMap<String, String>();
        sortedCondition.put(sortedObject, sortedValue);
        gongzuoliu.setSortedConditions(sortedCondition);
        QueryResult<Gongzuoliu> queryResult = gongzuoliuService.doPaginationQuery(gongzuoliu);
        JqGridPageView<Gongzuoliu> dictListView = new JqGridPageView<Gongzuoliu>();
        dictListView.setMaxResults(maxResults);
        List<Gongzuoliu> dictWithSubList = gongzuoliuService.queryDictWithSubList(queryResult.getResultList());
    ...
    public void doSave(Gongzuoliu entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
        if (CMD_EDIT.equals(parameter.getCmd())) {
            gongzuoliuService.merge(entity);
    ...
    public void operateDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (oper.equals("del")) {
            ...
            Gongzuoliu gongzuoliu = null;
            if (oper.equals("edit")) {
                gongzuoliu = gongzuoliuService.get(Long.valueOf(id));
            }
            Gongzuoliu dictKeyDict = gongzuoliuService.getByProerties("dictKey", dictKey);
            Gongzuoliu parentDictkeyDict = gongzuoliuService.getByProerties("dictKey", parentDictkey);
            if (StringUtils.isBlank(dictKey) || StringUtils.isBlank(dictValue)) {
                ...
                writeJSON(response, result);
            } else if (null != dictKeyDict && !gongzuoliu.getDictKey().equalsIgnoreCase(dictKey) && oper.equals("edit")) {
                response.setStatus(HttpServletResponse.SC_CONFLICT);
                result.put("message", "此字典编码已存在，请重新输入");
                writeJSON(response, result);
